...
#vpc_id=$(aws ec2 describe-vpcs --filters Name="tag:Name",Values="globo_primary" \
...
pub_subnet=$(aws ec2 create-subnet --availability-zone-id $az \
  --cidr-block "10.0.2.0/24" --vpc-id vpc-070f235d9aaa24a32 --output json)

# Create a new private subnet in the VPC
priv_subnet=$(aws ec2 create-subnet --availability-zone-id $az \
  --cidr-block "10.0.12.0/24" --vpc-id vpc-070f235d9aaa24a32 --output json)

# Create a private route table for priv_subnet
priv_rt=$(aws ec2 create-route-table --vpc-id vpc-070f235d9aaa24a32 --output json)
...
pub_rt_id=$(aws ec2 describe-route-tables --filters Name="vpc-id",Values=vpc-070f235d9aaa24a32 \
